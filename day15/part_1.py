...
cg = np.genfromtxt('day15/0.in', delimiter=1, dtype=int)

dijks_path = shortest_path(cg, (0, 0), (cg.shape[0] - 1, cg.shape[1] - 1))
print(f'Part 1 dijks: {dijks_path[1] - cg[(0, 0)]}')  # Don't count the first one

astar_path = shortest_path(cg, (0, 0), (len(cg) - 1, len(cg[0]) - 1), algo='astar')
print(f'Part 1 astar: {astar_path[1] - cg[(0, 0)]}')

cg_pt2 = np.zeros((len(cg) * 5, len(cg[0]) * 5), int)
for y in range(len(cg_pt2)):
    for x in range(len(cg_pt2[0])):
        ...
        cg_pt2[y, x] = (cg[y % cg.shape[0], x % cg.shape[1]] + y // cg.shape[0] + x // cg.shape[1] - 1) % 9 + 1
